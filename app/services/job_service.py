...
class JobService:
    ...
    def run_sync_and_export(self, trigger: str) -> JobResult:
        ...
        if trigger == SCHEDULED_TRIGGER and not sync_result.updated:
            msg = (
                "Scheduled run skipped export since no updates "
                f"(local={sync_result.local_epoch_ms}, remote={sync_result.remote_epoch_ms})."
            )
        ...
        if not sync_result.updated:
            msg = (
                f"No updates (local={sync_result.local_epoch_ms}, "
                f"remote={sync_result.remote_epoch_ms}). Manual run will still export."
            )
            log.info("Trigger=%s %s", trigger, msg)
        ...
        log.info("Trigger=%s Running export generation.", trigger)
